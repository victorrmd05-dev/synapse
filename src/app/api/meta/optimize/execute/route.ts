...
// optimization_goal e ajustes de posicionamento do bloco "execucao" do plano.
export async function POST(request: Request) {
  try {
    ...
    // 5. Duplica cada conjunto + seus anúncios
    for (const a of adsets) {
      try {
        ...
        // Targeting: clona o da fonte e aplica ajustes de posicionamento
        const targeting = a.targeting ? JSON.parse(JSON.stringify(a.targeting)) : { geo_locations: { countries: ['BR'] } };
        if (removerAN) targeting.publisher_platforms = ['facebook', 'instagram'];
